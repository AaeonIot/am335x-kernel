			  ...
			  int value)
{
	...
	if ((xr_usb_serial->DeviceProduct & 0xfff0) == 0x1400) {
		...
		result = usb_control_msg(
			xr_usb_serial->dev, /* usb device */
			usb_sndctrlpipe(xr_usb_serial->dev,
					0), /* endpoint pipe */
			  ...
			  short *value)
{
	...
	if ((xr_usb_serial->DeviceProduct & 0xfff0) == 0x1400) {
		...
		result = usb_control_msg(
			xr_usb_serial->dev, /* usb device */
			usb_rcvctrlpipe(xr_usb_serial->dev,
					0), /* endpoint pipe */
...
static struct xr21v141x_baud_rate xr21v141x_baud_rates[] = {
	...
	{ 0x100, 0x000, 0x100 }, { 0x020, 0x400, 0x020 },
	{ 0x010, 0x100, 0x010 }, { 0x208, 0x040, 0x208 },
	{ 0x104, 0x820, 0x108 }, { 0x844, 0x210, 0x884 },
	{ 0x444, 0x110, 0x444 }, { 0x122, 0x888, 0x224 },
	{ 0x912, 0x448, 0x924 }, { 0x492, 0x248, 0x492 },
	{ 0x252, 0x928, 0x292 }, { 0X94A, 0X4A4, 0XA52 },
	{ 0X52A, 0XAA4, 0X54A }, { 0XAAA, 0x954, 0X4AA },
	{ 0XAAA, 0x554, 0XAAA }, { 0x555, 0XAD4, 0X5AA },
	{ 0XB55, 0XAB4, 0X55A }, { 0X6B5, 0X5AC, 0XB56 },
	{ 0X5B5, 0XD6C, 0X6D6 }, { 0XB6D, 0XB6A, 0XDB6 },
	{ 0X76D, 0X6DA, 0XBB6 }, { 0XEDD, 0XDDA, 0X76E },
	{ 0XDDD, 0XBBA, 0XEEE }, { 0X7BB, 0XF7A, 0XDDE },
	{ 0XF7B, 0XEF6, 0X7DE }, { 0XDF7, 0XBF6, 0XF7E },
	{ 0X7F7, 0XFEE, 0XEFE }, { 0XFDF, 0XFBE, 0X7FE },
	{ 0XF7F, 0XEFE, 0XFFE }, { 0XFFF, 0XFFE, 0XFFD },
};
			      ...
			      unsigned int control)
{
	...
	if ((xr_usb_serial->DeviceProduct == 0x1410) ||
		...
		ret = xr_usb_serial_ctrl_msg(xr_usb_serial,
			   ...
			   struct usb_cdc_line_coding *line)
{
	...
	if ((xr_usb_serial->DeviceProduct == 0x1410) ||
		...
		xr_usb_serial_set_reg(
			xr_usb_serial, xr_usb_serial->reg_map.uart_format_addr,
			(format_size << 0) | (format_parity << 4) |
				(format_stop << 7));

	} else {
		ret = xr_usb_serial_ctrl_msg(xr_usb_serial,
					     USB_CDC_REQ_SET_LINE_CODING, 0,
					     line, sizeof *(line));
	}
	return ret;
}
				...
				struct tty_struct *tty, unsigned int cflag)
{
	...
	if (cflag & CRTSCTS) {
		...
		xr_usb_serial_set_reg(
			xr_usb_serial,
			xr_usb_serial->reg_map.uart_xoff_char_addr, stop_char);
	} else {
	...
	if ((xr_usb_serial->DeviceProduct == 0x1420) ||
	    (xr_usb_serial->DeviceProduct == 0x1422) ||
	    (xr_usb_serial->DeviceProduct ==
	     0x1424)) { //Add support for the TXT and RXT function for 0x1420, 0x1422, 0x1424, by setting GPIO_MODE [9:8] = '11'
		gpio_mode |= 0x300;
	}
...
int xr_usb_serial_send_break(struct xr_usb_serial *xr_usb_serial, int state)
{
	...
	if ((xr_usb_serial->DeviceProduct == 0x1410) ||
	    ...
	    (xr_usb_serial->DeviceProduct == 0x1414)) {
		if (state)
			ret = xr_usb_serial_set_reg(
				xr_usb_serial,
				xr_usb_serial->reg_map.tx_break_addr, 0xffff);
		else
			ret = xr_usb_serial_set_reg(
				xr_usb_serial,
				xr_usb_serial->reg_map.tx_break_addr, 0);
	} else {
		ret = xr_usb_serial_ctrl_msg(
			xr_usb_serial, USB_CDC_REQ_SEND_BREAK, state, NULL, 0);
	}
	return ret;
}
...
int xr_usb_serial_enable(struct xr_usb_serial *xr_usb_serial)
{
	...
	if ((xr_usb_serial->DeviceProduct == 0x1410) ||
	    (xr_usb_serial->DeviceProduct == 0x1412) ||
	    (xr_usb_serial->DeviceProduct == 0x1414)) {
		ret = xr_usb_serial_set_reg_ext(xr_usb_serial, URM_REG_BLOCK,
						URM_ENABLE_BASE + channel,
						URM_ENABLE_0_TX);
		ret = xr_usb_serial_set_reg(
			xr_usb_serial, xr_usb_serial->reg_map.uart_enable_addr,
			UART_ENABLE_TX | UART_ENABLE_RX);
		ret = xr_usb_serial_set_reg_ext(
			xr_usb_serial, URM_REG_BLOCK, URM_ENABLE_BASE + channel,
	...
	if ((xr_usb_serial->DeviceProduct == 0x1410) ||
		...
		case 0:
			ret = xr_usb_serial_set_reg_ext(
				...
				xr_usb_serial->reg_map.uart_loopback_addr,
				0x40);
			break;
		case 1:
			ret = xr_usb_serial_set_reg_ext(
...
int xr_usb_serial_set_wide_mode(struct xr_usb_serial *xr_usb_serial,
				int preciseflags)
{
	int ret = 0;
	int channel = xr_usb_serial->channel;
	...
	if ((xr_usb_serial->DeviceProduct & 0xfff0) == 0x1400) {
		...
		if (channel)
			channel--;
		xr_usb_serial_set_reg_ext(
			xr_usb_serial, 0x66,
			channel * 8 + XR21V1414_WIDE_MODE_OFFSET, preciseflags);
		...
		xr_usb_serial_set_reg(xr_usb_serial, 0xd02, preciseflags);
		   ...
		   (xr_usb_serial->DeviceProduct == 0x1424)) {
		xr_usb_serial_set_reg(xr_usb_serial,
				      XR21B142X_WIDE_MODE_TX_OFFSET,
				      preciseflags);
		xr_usb_serial_set_reg(xr_usb_serial,
				      XR21B142X_WIDE_MODE_RX_OFFSET,
				      preciseflags);
	}
	xr_usb_serial_enable(xr_usb_serial);
	return ret;
...

{
	short data;
	int result;
	result = xr_usb_serial_get_reg(
		xr_usb_serial, xr_usb_serial->reg_map.uart_gpio_status_addr,
		&data);
