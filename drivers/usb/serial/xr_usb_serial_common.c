...
{
	int retval = usb_control_msg(
		xr_usb_serial->dev, usb_sndctrlpipe(xr_usb_serial->dev, 0),
 ...
 */
static ssize_t show_caps(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
...
static ssize_t show_country_codes(struct device *dev,
...
static ssize_t show_country_rel_date(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev);
	struct xr_usb_serial *xr_usb_serial = usb_get_intfdata(intf);
					  ...
					  gfp_t mem_flags)
{
	...
	for (i = 0; i < xr_usb_serial->rx_buflimit; ++i) {
		res = xr_usb_serial_submit_read_urb(xr_usb_serial, i,
						    mem_flags);
		if (res)
			return res;
	}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0)
			tty_insert_flip_char(&xr_usb_serial->port, ch,
					     tty_flag);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0)
		tty_insert_flip_string(&xr_usb_serial->port,
				       urb->transfer_buffer,
				       urb->actual_length);
		tty_flip_buffer_push(&xr_usb_serial->port);
#else
				       ...
				       struct tty_struct *tty)
{
	...
	if (xr_usb_serial_set_control(xr_usb_serial, xr_usb_serial->ctrlout) <
		    0 &&
				   ...
				   const unsigned char *buf, int count)
{
	...
	count = (count > xr_usb_serial->writesize) ? xr_usb_serial->writesize :
							   count;
...
static int xr_usb_serial_tty_write_room(struct tty_struct *tty)
{
	 ...
	 */
	return xr_usb_serial_wb_is_avail(xr_usb_serial) ?
...
static int xr_usb_serial_tty_chars_in_buffer(struct tty_struct *tty)
{
	 ...
	 */
	return (XR_USB_SERIAL_NW - xr_usb_serial_wb_is_avail(xr_usb_serial)) *
	       xr_usb_serial->writesize;
}
...
static int xr_usb_serial_tty_tiocmset(struct tty_struct *tty, unsigned int set,
				      unsigned int clear)
{
	struct xr_usb_serial *xr_usb_serial = tty->driver_data;
			   ...
			   struct serial_struct __user *info)
{
	...
	tmp.closing_wait =
		xr_usb_serial->port.closing_wait == ASYNC_CLOSING_WAIT_NONE ?
...
static int xr_usb_serial_tty_ioctl(struct tty_struct *tty, unsigned int cmd,
				   unsigned long arg)
{
	struct xr_usb_serial *xr_usb_serial = tty->driver_data;
	int rv = -ENOIOCTLCMD;
	...
	case XR_USB_SERIAL_SET_GPIO_MODE_REG:
		...
		if (channel == -1) {
			...
			rv = xr_usb_serial_set_reg(
				xr_usb_serial,
				xr_usb_serial->reg_map.uart_gpio_mode_addr,
				val);
		} else {
			rv = xr_usb_serial_set_reg_ext(
				xr_usb_serial, channel,
				xr_usb_serial->reg_map.uart_gpio_mode_addr,
				val);
		}

		dev_dbg(&xr_usb_serial->control->dev,
			"XR_USB_SERIAL_SET_GPIO_MODE_REG 0x%x val:0x%x \n",
	...
	case XR_USB_SERIAL_GET_GPIO_MODE_REG:
		...
		if (channel == -1) {
			rv = xr_usb_serial_get_reg(
				xr_usb_serial,
				xr_usb_serial->reg_map.uart_gpio_mode_addr,
				data);
		} else {
			rv = xr_usb_serial_get_reg_ext(
				xr_usb_serial, channel,
				xr_usb_serial->reg_map.uart_gpio_mode_addr,
				data);
		}
		...
		dev_dbg(&xr_usb_serial->control->dev,
			"XR_USB_SERIAL_GET_GPIO_MODE_REG 0x%x val:0x%x \n",
			xr_usb_serial->reg_map.uart_gpio_mode_addr, *data);

		if (rv < 0) {
			dev_err(&xr_usb_serial->control->dev,
				"Cannot get register (%d) channel=%d \n", rv,
				channel);
			kfree(data);
			return -EFAULT;
		}
	...
	case XRIOC_SET_ANY_BAUD_RATE:
		...
		memcpy(&newline, &(xr_usb_serial->line),
		       sizeof(struct usb_cdc_line_coding));
		xr_usb_serial_disable(xr_usb_serial);
		rv = xr_usb_serial_set_line(xr_usb_serial, &newline);
		xr_usb_serial_enable(xr_usb_serial);
		dev_dbg(&xr_usb_serial->control->dev,
			"XRIOC_SET_ANY_BAUD_RATE set baud_rate:%d ret=%d\n",
			baud_rate, rv);
		break;
	case XRIOC_SET_PRECISE_FLAGS:
		preciseflags = arg;
		dev_dbg(&xr_usb_serial->control->dev,
			"%s VIOC_SET_PRECISE_FLAGS %d\n", __func__,
			preciseflags);
		xr_usb_serial_disable(xr_usb_serial);
		if (preciseflags) {
		...
		xr_usb_serial_set_wide_mode(xr_usb_serial,
					    xr_usb_serial->preciseflags);
		xr_usb_serial_enable(xr_usb_serial);
		break;
	}
					  ...
					  struct ktermios *termios_old)
{
	...
	xr_usb_serial_set_flow_mode(xr_usb_serial, tty,
				    cflag); /*set the serial flow mode*/
...
static void
xr_usb_serial_write_buffers_free(struct xr_usb_serial *xr_usb_serial)
{
	int i;
	struct xr_usb_serial_wb *wb;
...
static int
xr_usb_serial_write_buffers_alloc(struct xr_usb_serial *xr_usb_serial)
{
	int i;
	struct xr_usb_serial_wb *wb;

	for (wb = &xr_usb_serial->wb[0], i = 0; i < XR_USB_SERIAL_NW;
	     i++, wb++) {
		wb->buf = usb_alloc_coherent(xr_usb_serial->dev,
					     xr_usb_serial->writesize,
					     GFP_KERNEL, &wb->dmah);
		if (!wb->buf) {
			while (i != 0) {
...
made_compressed_probe:
	...
	for (i = 0; i < num_rx_buf; i++) {
		...
		rb->base = usb_alloc_coherent(xr_usb_serial->dev, readsize,
					      GFP_KERNEL, &rb->dma);
		if (!rb->base) {
		...
		if (xr_usb_serial->is_int_ep) {
			usb_fill_int_urb(urb, xr_usb_serial->dev,
					 xr_usb_serial->rx_endpoint, rb->base,
...
#else
	tty_dev = tty_port_register_device(&xr_usb_serial->port,
...
static void xr_usb_serial_disconnect(struct usb_interface *intf)
{
	...
	if (!xr_usb_serial->combined_interfaces)
		usb_driver_release_interface(&xr_usb_serial_driver,
					     intf == xr_usb_serial->control ?
							   xr_usb_serial->data :
							   xr_usb_serial->control);
...
static int __init xr_usb_serial_init(void)
{
	...
	xr_usb_serial_tty_driver->init_termios.c_cflag =
